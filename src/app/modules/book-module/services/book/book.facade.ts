...
export class BookFacadeService {
    ...
    (
        ...
        return this.http.get<BookResult>(`${environment.volumes}?q=${name}&maxResults=${maxResults}&startIndex=${startIndex}`).pipe(
            map(response => {
    ...
    loadBooksByCategory(
        ...
        return this.http.get<BookResult>(`${environment.volumes}?q=${category}&maxResults=${maxResults}&startIndex=${startIndex}`).pipe(
            map(response => {
            const items = response.items || [];
            const mappedItems: BooksView[] = items.map(item => ({
                id: item.id,
    ...
    bookById(id: string): Observable<BookDetails> {
        return this.http.get<BookDetailsResult>(`${environment.volumes}/${id}`, {
